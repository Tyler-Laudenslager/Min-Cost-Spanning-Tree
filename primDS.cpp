...
#include <functional>
#include "Heap_ADT.h"
#include "Edge.h" // To use std::greater<T> -> This will prove to be useful in picking the minimum weight
using namespace std;
...
/*! \brief Creates and adds the edge to the adjacency list
    \param adjList reference to a array that has holds vector<Edge>
...
void createAndAddEdge(vector <Edge> adjList[], int vertex1, int weight, int vertex2){
  adjList[vertex1-97].push_back(Edge(vertex1, weight, vertex2));
  adjList[vertex2-97].push_back(Edge(vertex2, weight, vertex1)); // do this because undirected graph
} // createAndAddEdge(vector <int> List[], int u, int v, int weight)

/*! \brief Finds the minimum weight spanning tree of an adjancency list.
    \param adjList reference to an adjancency list.
    \param Edge_List refernce to a binary heap of chosen edges
...
int PrimsAlgorithm(vector <Edge> adjList[], HeapADT<Edge> &Edge_List, int source, int vertices){
  vector <bool> visitedVertex(vertices, false);
  HeapADT<Edge> PQ; // Set up priority queue
  Edge edge;
  int minCost = 0;
  PQ.insertToHeap(Edge(source,0,source)); // Source has weight 0;
  while (!PQ.isHeapEmpty()){
    PQ.removeFromHeap(edge); // Use to get minimum weight
    ...
    if (edge.weight != 0) {
        Edge_List.insertToHeap(edge);
    }
    minCost += edge.weight; // Add to minCost
    for (vector<Edge>::iterator it = adjList[source].begin(); it != adjList[source].end(); it++) {
      int index = (int) (it->vertexB) - 97;// Visit all children
      if (!visitedVertex.at(index)) // If vertex hasn't been visited already
        PQ.insertToHeap(Edge(it->vertexA, it->weight, it->vertexB)); // Push vertex and weight onto Priority Queue
    }
...
void print_spanning_tree(HeapADT<Edge> &edge_list, int cost, char source) {
  cout << endl;
  cout << "minimum spanning tree -> start " << (char) (source) << endl;
  while(!edge_list.isHeapEmpty()) {
      Edge edge_struct;
      edge_list.removeFromHeap(edge_struct);
      cout << "edge (" << edge_struct.vertexA << "," << edge_struct.vertexB << ") weight : " << edge_struct.weight << endl;
  }
...
/*! \brief Parses the contents from an opened file.
    ...
    \param adj_list reference to an array that contains vector<Edge> as elements
    \returns N/A
...
bool get_info_from_file(ifstream &infile, vector<Edge> adj_list[]) {
...
int main (int argc, char *argv[]) {
     ...
     if (open_file_to_read(argc, argv, infile)) {
       ...
       vector<Edge> adjList[num_vertices];
       ...
       while (true) {
         ...
         cout << "data structure algorithm took " << total_ticks.count() << " microseconds." << endl;
